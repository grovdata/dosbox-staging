...
static std::optional<HostLanguage> host_language         = {};
...
void MSG_Init([[maybe_unused]] Section_prop *section)
{
	...
	if (!host_language) {
		host_language = DetectHostLanguage();
	}
	...
	if (language_file.empty()) {
		...
		assert(section);
		language_file = static_cast<const Section_prop*>(section)->Get_string("language");
	}
	...
	if (!language_file.empty() && language_file != "auto") {
		if (language_file.ends_with(extension)) {
			...
			// If a short-hand name was provided, add the file extension
	...
	if (host_language->language_file == "en") {
		LOG_MSG("LANG: Using internal English language messages "
			"(detected from host value '%s')",
			host_language->log_info.c_str());
		return;
	}

	// Try to load autodetected language
	file_path = GetResourcePath(subdirectory, host_language->language_file + extension);
	const auto result = load_message_file(file_path);
